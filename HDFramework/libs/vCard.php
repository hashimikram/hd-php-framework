...
class vCard implements \Countable, \Iterator
{
    ...
    private static $Spec_MultipleValueElements = array(
        'nickname',
        'categories'
    );
    ...
    private static $Spec_FileElements = array(
        'photo',
        'logo',
        'sound'
    );
     ...
     *
     * @param
     *            string Path to file, optional.
     * @param
     *            string Raw data, optional.
     * @param
     *            array Additional options, optional. Currently supported options:
     *            bool Collapse: If true, elements that can have multiple values but have only a single value are returned as that value instead of an array
     *            If false, an array is returned even if it has only one value.
     *
    ...
    public function __construct($Path = false, $RawData = false, array $Options = null)
    {
        ...
        if ($this->Mode == self::MODE_MULTIPLE) {
            ...
            foreach ($Lines as $Line) {
                ...
                if (in_array($Key, self::$Spec_FileElements) && isset($Parameters['encoding']) && in_array($Parameters['encoding'], array(
                    'b',
                    'base64'
                ))) {
     ...
     *            string Key
     * @param
     *            int Index of the file, defaults to 0
     * @param
     *            string Target path where the file should be saved, including the filename
     *
     ...
     *            string Key
     * @param
     *            string Method call arguments. First element is value.
     *
